...
// Fetch rejected bookings with event names
$rejectedBookings = $conn->query("SELECT id, name, email, date, event, booking_number, status FROM appointments WHERE status='rejected'");

// Search functionality
$searchResult = null;
if (isset($_POST['search'])) {
    $searchTerm = $conn->real_escape_string($_POST['search_term']);
    $searchResult = $conn->query("SELECT id, name, email, date, event, booking_number, status FROM appointments WHERE (name LIKE '%$searchTerm%' OR email LIKE '%$searchTerm%')");
}
...
if (isset($_POST['reject_booking'])) {
    $bookingId = $_POST['booking_id'];
    $conn->query("UPDATE appointments SET status='rejected' WHERE id=$bookingId");
    header("Location: admin_dashboard.php");  // Refresh the page after rejection
}
    ...
    <div class="main-content">
                        ...
                        <tr>
                            ...
                            <td class="booking-actions">
                                <form method="POST">
                                    ...
                                    <button type="submit" name="reject_booking" class="button delete-button">Reject</button>
                                </form>
                            </td>
                        </tr>
                    <?php endwhile; ?>
        ...
        <section>
            ...
            <table>
                ...
                <tbody>
                    <?php while ($row = $rejectedBookings->fetch_assoc()): ?>
                        <tr>
                            <td><?= $row['booking_number']; ?></td>
                            <td><?= $row['name']; ?></td>
                            <td><?= $row['email']; ?></td>
